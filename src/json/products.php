	...
	{		
		public function printJSON()
		{
			if($query = $this->coffee->checkPassword()) {
				...
				$query = $this->coffee->db()->prepare("SELECT p.name AS name, p.id AS id, COUNT(t.id) AS amount FROM Products p LEFT JOIN Transactions t ON t.product = p.id AND t.user = ? AND t.date >= ? GROUP BY p.id");
